package com.snapmaker.fabscreen.viewmodel;
...
//            getModel().getNetworkController().save(mSelectedAccessPoint.getSSID(), password);
    ...
    public boolean isConnected() {
        ...
        return (active != NULL_ACCESS_POINT
                && selected != NULL_ACCESS_POINT
    ...
    public Observable<AccessPoint> getNotIdleAccessPointObservable() {
        return mAccessPointsSubject.flatMap(accessPoints -> {
            ...
            for (AccessPoint accessPoint : accessPoints) {
                ConnectState status = accessPoint.getConnectState();
                if (status == ConnectState.SELECTED) selectedAP = accessPoint;
                if (status == ConnectState.CONFIRMED) userConfirmedAP = accessPoint;
                if (status == ConnectState.CONNECTING) connectingAP = accessPoint;
                if (status == ConnectState.CONNECTED) connectedAP = accessPoint;
            }
            ...
            return Observable.just(NULL_ACCESS_POINT);
        });
    }
