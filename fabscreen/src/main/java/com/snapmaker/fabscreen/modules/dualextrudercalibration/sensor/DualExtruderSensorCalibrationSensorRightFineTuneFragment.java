...
public class DualExtruderSensorCalibrationSensorRightFineTuneFragment extends BaseFragment {
    ...
    public static Fragment newInstance() {
        return new DualExtruderSensorCalibrationSensorRightFineTuneFragment();
    }
    ...
    private void initView() {
        setTitle(R.string.calibration_dual_extruder_sensor_calibration_right_probe_title);
    ...
    void onClickMoveUp() {
        if (!mIsValueModifiedSubject.getValue()) {
            mIsValueModifiedSubject.onNext(true);
        }
    ...
    void onClickSave() {
        enableButtons(false);
        mActivityViewModel.saveSensorFineTuneResult(1)
                .observeOn(AndroidSchedulers.mainThread())
                .as(bindToLifecycle())
                .subscribe(success -> {
                    enableButtons(true);
                    if (success) {
                        // Route to right fine tune complete.
                        ((DualExtruderSensorCalibrationActivity) requireActivity()).gotoRightExtruderFineTuneComplete();
                    } else {
                        // Failed.
                    }
                }, e -> {
