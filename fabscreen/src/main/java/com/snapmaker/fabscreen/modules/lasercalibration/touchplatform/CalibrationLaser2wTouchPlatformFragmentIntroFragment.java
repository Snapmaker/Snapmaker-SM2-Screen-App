...
public class CalibrationLaser2wTouchPlatformFragmentIntroFragment extends BaseFragment {
    public static CalibrationLaser2wTouchPlatformFragmentIntroFragment newInstance() {
        return new CalibrationLaser2wTouchPlatformFragmentIntroFragment();
    }
        ...
        getModel().getPrintController().getHeaderSecurityStatus().observeOn(AndroidSchedulers.mainThread())
                ...
                .subscribe(headerSecurity -> {
                    if (headerSecurity.status == 0) {
                        ...
                        ((LaserCalibrationActivity) requireActivity()).start2wPlatformHeightPullOutFocusLever();
                    }
                });
    }
}
