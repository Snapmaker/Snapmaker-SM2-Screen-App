...
public class ControlDualExtruder3DPFragment extends BaseFragment {
    ...
    View mViewPageDualExtruderNozzle;
    ...
    private void initViewPager() {
        ...
        views.add(mViewPageDualExtruderNozzle);
        ...
        mControlViewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                if (position == 1) {
                    ...
                    if (machineStatus.headTargetTemperature == 0 && machineStatus.extruder1TargetTemperature == 0) {
                        FabConfirm.create(getContext())
                                .setDescription(R.string.control_heat_warning)
                                .setConfirm(R.string.all_confirm, (dialog, which) -> {
                                    dialog.dismiss();
                                    mDualExtruderNozzleWidgetPresenter.preheatedTemperature();
                                })
    ...
    private void initNozzlePage() {
        ...
        mDualExtruderNozzleWidgetPresenter.bind(mViewPageDualExtruderNozzle);
        mDualExtruderNozzleWidgetPresenter.connectStatus();

//        mDualExtruderNozzleWidgetPresenter.getIsLoadingObservable()
...
//                .subscribe(isLoading -> mBtnBack.setVisibility(isLoading ? View.INVISIBLE : View.VISIBLE));
    ...
    private Observable<Boolean> turnOffHead() {
        FabPacketContent.MachineStatus machineStatus = getModel().getSlaveComputer().getMachineStatus();
        if (machineStatus.headTargetTemperature > 0 || machineStatus.extruder1TargetTemperature > 0) {
            return getModel().getSlaveComputer().sendGcode("M104 T0 S0")
                    .flatMap(response -> getModel().getSlaveComputer().sendGcode("M104 T1 S0"))
                    .map(response -> true);
        } else {
