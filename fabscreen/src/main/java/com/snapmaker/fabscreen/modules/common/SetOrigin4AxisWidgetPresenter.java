...
public class SetOrigin4AxisWidgetPresenter extends BasePresenter {
    ...
    @BindView(R.id.btn_control_laser_page_set_origin_set_origin_b)
    ...
    public SetOrigin4AxisWidgetPresenter(Context context, Model model, CompositeDisposable compositeDisposable) {
    ...
    @OnClick(R.id.btn_control_laser_page_set_origin_set_origin_b)
    void onClickSetOriginB() {
        ...
        Disposable sub = getModel().getSlaveComputer().setPosition(0, 0, 0, 0, ISlaveComputer.FLAG_B)
                .flatMap(this::updateCoordinateSystem)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(success -> {
                    mMovingEventSubject.onNext(false);
    ...
    void onClickSetOrigin() {
        ...
        Disposable sub = getModel().getSlaveComputer().setPosition(0, 0, 0, 0, ISlaveComputer.FLAG_XYZB)
                .flatMap(this::updateCoordinateSystem)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(success -> {
                    mMovingEventSubject.onNext(false);
    ...
    void onClickGotoOrigin() {
        ...
        if (currentZ > 0) {
            ...
            Disposable sub = getModel().getSlaveComputer().sendGcode("G1 X0 Y0 F3000")
            ...
            Disposable sub = getModel().getSlaveComputer().sendGcode("G1 Z0 F1800")
