...
public class PreviewLaser2wPullInFocusLeverFragment extends BaseFragment {
    public static PreviewLaser2wPullInFocusLeverFragment newInstance() {
        return new PreviewLaser2wPullInFocusLeverFragment();
    }
    ...
    private void initView() {
        Glide.with(this)
                .load(R.drawable.pic_laser_2w_focus_lever_pull_in_360x320)
                .into(mIvCover);
    ...
    void onClickNext() {
        ...
        if (getModel().getMachineController().isRotaryModuleAvailable()) {
            ((PreviewActivity) requireActivity()).gotoLaser2wPrepareSafetyGogglesFragment(autoMode);
            return;
        }
        ((PreviewActivity) requireActivity()).gotoLaser2wPrepareSetOriginFragment(autoMode);
    }
}
