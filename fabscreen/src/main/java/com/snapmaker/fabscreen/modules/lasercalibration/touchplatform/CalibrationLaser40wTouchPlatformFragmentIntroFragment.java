...
public class CalibrationLaser40wTouchPlatformFragmentIntroFragment extends BaseFragment {
    public static CalibrationLaser40wTouchPlatformFragmentIntroFragment newInstance() {
        return new CalibrationLaser40wTouchPlatformFragmentIntroFragment();
    }
        ...
        getModel().getPrintController().getHeaderSecurityStatus().observeOn(AndroidSchedulers.mainThread())
                ...
                .subscribe(headerSecurity -> {
                    if (headerSecurity.status == 0) {
                        ...
                        ((LaserCalibrationActivity) requireActivity()).start40wPlatformHeightPullOutFocusLever();
                    }
                });
    }
}
