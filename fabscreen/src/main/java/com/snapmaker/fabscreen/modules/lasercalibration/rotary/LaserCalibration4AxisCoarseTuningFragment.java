...
public class LaserCalibration4AxisCoarseTuningFragment extends BaseFragment {
    ...
    @BindView(R.id.btn_laser_calibration_4axis_coarse_turning_next)
    Button mBtnNext;
    ...
    private Observable<Boolean> setAsOrigin(boolean success) {
        ...
        return getModel().getMachineController().updateCoordinateSystem(1)
                .flatMap(response -> getModel().getSlaveComputer().setPosition(0, 0, 0, 0, ISlaveComputer.FLAG_XYZB))
    ...
    @OnClick(R.id.btn_laser_calibration_4axis_coarse_turning_next)
    void onClickNext() {
        float bottomZ = getModel().getPreferences().getLaserBottomZ();
