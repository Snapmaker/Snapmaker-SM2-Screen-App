...
public class DualExtruderZOffsetWidgetPresenter extends BasePresenter {
    ...
    private int mExtruder = 0;
    private BehaviorSubject<Float> mZOffsetSubject = BehaviorSubject.createDefault(0f);
    private float mMoveStep = 0.1f;

    public DualExtruderZOffsetWidgetPresenter(Context context, Model model, CompositeDisposable compositeDisposable) {
        super(context, model, compositeDisposable);
    }

    public void bind(int which, View view) {
        ...
        mTvTitle.setText(which == 0 ? getContext().getString(R.string.print_left_z_offset): getContext().getString(R.string.print_right_z_offset));
        // changes
    ...
    public void connectPrintSettings() {
        float initialValue = getModel().getPrintController().getOverrideZOffset(mExtruder);
    ...
    public void connectPrint(int which, float zOffset) {
        if (mExtruder == which) {
            setZOffset(zOffset);
        }
    }
    ...
    void onClickUp() {
        ...
        getModel().getPrintController().setOverrideZOffset(mExtruder, targetValue);
    }
