...
public class Settings10wCameraCalibrationViewModel extends BaseViewModel {
    ...
    public Settings10wCameraCalibrationViewModel() {
        ...
        switch (getModel().getPreferences().getMachineModel()) {
            ...
            case Constants.MACHINE_TYPE_A350:
                ...
                break;
            default:
                mTakePhotoPoint = new Point(getModel().getMachineController().getSizeX() / 2, getModel().getMachineController().getSizeY() / 2);
                cameraZ = 140;
                d = 80;
        }
        mCenter = new Point(getModel().getMachineController().getSizeX() / 2, getModel().getMachineController().getSizeY() / 2);
    ...
    private void initPosition() {
        ...
        final float bottomZ = getModel().getPreferences().getLaserPlatformZ();
        ...
        getModel().getMachineController().updateCoordinateSystem()
                ...
                .subscribe(coordinateSystem -> {
                    ...
                    if (!homed) {
                        ...
                        getModel().getSlaveComputer().gotoAbsolutePosition(mCenter.x, mCenter.y, mZPos)
                                .flatMap(success -> getModel().getSlaveComputer().gotoAbsolutePosition(lastPoint.x, lastPoint.y, mZPos))
                                .observeOn(AndroidSchedulers.mainThread())
                                .as(bindToLifecycle())
                                .subscribe(success -> {
    ...
    private Observable<Boolean> engraveLine(Point point, int index) {
        final int x = point.x;
        final int y = point.y;
        ...
        return getModel().getSlaveComputer().sendGcode("M3 P20")
                .flatMap(response -> getModel().getSlaveComputer().gotoAbsolutePosition(engraveX, engraveY, mZPos, 300))
                .flatMap(success -> getModel().getSlaveComputer().sendGcode("M5"))
                .flatMap(success -> getModel().getSlaveComputer().gotoAbsolutePosition(x, y, mZPos, 300));
    }

    private void detectingRectangle() {
        getModel().getSlaveComputer().gotoAbsolutePosition(mTakePhotoPoint.x, mTakePhotoPoint.y, cameraZ)
                .flatMap(success -> getModel().getLaserCameraController().setPhotoQuality(10))
                ...
                .subscribe(bitmap -> {
                    ...
                    String path = getModel().getCacheDir() + "/10WLaserCalibration.jpg";
...
//        saveJpg(cropped, mIndex++ + ".jpg");
    ...
    private void saveJpg(Bitmap cropped, String name) {
        try {
            File file = new File(getModel().getFilesDir(), name);
    ...
    private void process(Bitmap image) {
        ...
        getModel().getPreferences().set10WLaserCameraCalibration(result.toString());
        int headerId = (int) (Math.random() * Integer.MAX_VALUE);
        Logger.d("1.14.0 Fix BUG headerId: " + headerId);
        getModel().getSlaveComputer().setHeaderOnlineSyncId(headerId)
                .observeOn(AndroidSchedulers.mainThread())
                .as(bindToLifecycle())
                .subscribe(success -> {
                    if (success) {
                        getModel().getPreferences().setHeaderOnlineSyncID(headerId);
                    } else {
